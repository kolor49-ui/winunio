...
export function UserPushSetup() {
  ...
  async function enablePush() {
    ...
    setStatus(null);
    try {
      const configRes = await fetch("/api/v1/push/subscribe");
      ...
      setStatus(
        saveData.message ??
          "Push értesítés engedélyezve — értesítünk, ha téged érintő esemény történik.",
      );
    } catch (err) {
  ...
  if (!supported) {
    return (
      <p className="hint">
        A böngésződ nem támogatja a push értesítést. A vitáidhoz kapcsolódó
        fontos eseményekről e-mailben is kaphatsz értesítést, ha az adott
